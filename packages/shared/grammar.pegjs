...
BeginToken      = "BEGIN"      !IdentifierPart
...
ShiftExpression
  = head:AdditiveExpression { return buildBinaryExpression(head, []); }
...
ArgumentList
  = head:(FunctionCall / AssignmentExpression) tail:(__ "," __ (FunctionCall / AssignmentExpression))* {
      return buildList(head, tail, 3);
    }

Expression
  = head:(FunctionCall / AssignmentExpression) {
  		return head;
    }
...
Block
  = BeginToken __ body:(StatementList __)? __ _return:ReturnStatement __ EndToken {
      return {
        type: "BlockStatement",
        body: [...optionalList(extractOptional(body, 0)), _return]
      };
    }

StatementList
  = head:SimpleStatement tail:(__ SimpleStatement)* { return buildList(head, tail, 1); }
...
ExpressionStatement
  = !(BeginToken / ReturnToken) expression:Expression EOS {
      return {
